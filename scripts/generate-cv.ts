...
const DEFAULT_OUTPUT_PATH = "build/cv/ZixiangWang_CV.tex";
const HOMEPAGE_URL =
...
\hypersetup{
    ...
    linkcolor=darkred,
    urlcolor=darkred,
    pdftitle={${tex(siteData.profile.name)}'s CV},
    pdfauthor={${tex(siteData.profile.name)}},
    pageanchor=false
}
\urlstyle{same}
...
{\small ${tex(htmlToText(siteData.profile.bio))}\par}
...
function getVenueShort(venue = "") {
  const candidates = [
    ...
    { pattern: /\bNeurIPS\b|Neural Information Processing Systems/i, value: "NeurIPS" },
    ...
    { pattern: /STAR Protocols/i, value: "STAR Protocols" },
